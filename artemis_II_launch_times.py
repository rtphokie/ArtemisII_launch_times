...
try:
    ...
    raise ImportError(
        "Requires 'skyfield' and 'jplephem'. Install with: pip install skyfield jplephem"
    ) from exc
...
def moonrise_for_date(
    date_or_dt: Union[date, datetime],
    latitude: float = KSC_LAT,
    longitude: float = KSC_LON,
) -> Optional[datetime]:
    """
...
def load_launch_windows_from_csv(csv_path: str):
    ...
    with open(csv_path, "r") as f:
        ...
        for row in reader:
            dt_start_utc = datetime.fromisoformat(row["window_start_utc"]).replace(
                tzinfo=timezone.utc
            )
            duration = int(row["duration_mins"])
            windows.append(
                {"window_start_utc": dt_start_utc, "duration_mins": duration}
            )
    return windows
...
def calculate_moon_positions_for_launch_windows(
    csv_path: str = None, latitude=KSC_LAT, longitude=KSC_LON, local_tz: str = KSC_TZ
):
    """
...
if __name__ == "__main__":
    ...
    for n, window in enumerate(calculate_moon_positions_for_launch_windows()):
        ...
        print(
            f" ({moonrisedelta.total_seconds() / 60:3.0f} into window, {window['moon_illumination']:3.0f}% illuminated)",
            end=" ",
        )
